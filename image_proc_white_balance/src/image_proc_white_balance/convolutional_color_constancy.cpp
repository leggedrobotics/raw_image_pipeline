...
void ConvolutionalColorConstancyWB::balanceWhite(const cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst) {
  ...
  //     cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_src" + std::to_string(idx_) + ".png", src_vis);

  //     cv::Mat dst_vis;
  //     cv::normalize(dst, dst_vis, 0, 255.0, cv::NORM_MINMAX);
  //     cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_dst" + std::to_string(idx_) + ".png", dst_vis);
  // }
  idx_++;
}
#endif
...
int ConvolutionalColorConstancyWB::loadModel(const std::string& model_file) {
  ...
  if (debug_) {
    ...
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_filter" + std::to_string(idx_) + ".png", filter_vis);

    cv::Mat bias_vis;
    cv::normalize(model_.bias_, bias_vis, 0, 255.0, cv::NORM_MINMAX);
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_bias" + std::to_string(idx_) + ".png", bias_vis);
  }
...
void ConvolutionalColorConstancyWB::calculateHistogramFeature(const cv::Mat& src, std::string out_name) {
  ...
  if (debug_) {
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_lower_mask_" + out_name + std::to_string(idx_) + ".png", lower_mask);
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_upper_mask_" + out_name + std::to_string(idx_) + ".png", upper_mask);
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_mask_" + out_name + std::to_string(idx_) + ".png", mask);
  }
  ...
  if (debug_) {
    ...
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_histogram_" + out_name + std::to_string(idx_) + ".png", hist_vis);
  }
}
...
int ConvolutionalColorConstancyWB::computeResponseCuda() {
  ...
  if (debug_) {
    ...
    cv::imwrite(FILE_FOLDER + "/image_proc_white_balance_response" + std::to_string(idx_) + ".png", gpu_response_vis);
  }
  return 0;
}
